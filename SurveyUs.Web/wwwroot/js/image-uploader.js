    ...
    $.fn.imageUploader = function (options) {
        ...
        let createContainer = function () {
            ...
            let $uploadedContainer = $('<div>', { class: 'uploaded' }).appendTo($container),
                ...
                $i = $('<i>', { class: 'fas fa-cloud-upload blue-text-400' }).appendTo($textContainer),
        ...
        let createImg = function (src, id, preloaded, filenamez) {

            // Create the upladed image container
            let $container = $('<div>', { class: 'uploaded-image' }),

                // Create the img tag
                $img = $('<img>', { src: src, alt: filenamez, title: filenamez }).appendTo($container),
                // Create the delete button
                $button = $('<button>', { class: 'delete-image' }).appendTo($container),

                // Create the delete icon
                $i = $('<i>', { class: 'far fa-times-circle' }).appendTo($button);
        ...
        let setPreview = function ($container, files) {
            ...
            $(files).each(function (i, file) {
                ...
                if (file.type == "application/pdf") {
                    var sos = "/Plugins/MyTVO.Widgets.MyDocument/Content/images/document.png";
                    $uploadedContainer.append(createImg(sos, dataTransfer.items.length - 1, false, file.name));
