...
using SurveyUs.Application.Features.Logs.Commands.AddActivityLog;
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    else
                    {
                        ...
                        if (userRole != null)
                        {
                            if (userRole.Contains("Bartender") || userRole.Contains("Heineken Judge") || userRole.Contains("Guinness Judge") && !(userRole.Contains("SuperAdmin") || userRole.Contains("Admin")))
                                password = Input.Password + "Abcd1234";
                        }

                        var result = await _signInManager.PasswordSignInAsync(userName, password, Input.RememberMe, lockoutOnFailure: false);
                        if (result.Succeeded)
                        {
